...
var PlayLayer = cc.Layer.extend({
    ...
    init:function () {
        ...
        cc.spriteFrameCache.addSpriteFrames(res.s_plist_parkour);
        this.spriteSheet = cc.SpriteBatchNode.create(res.s_img_parkour);
    ...
    onTouchEnded:function(touch, event) {
        ...
        switch (rtn) {
            case "up":
                event.getCurrentTarget().runner.jump();
                break;
            case "down":
                event.getCurrentTarget().runner.crouch();
                break;
            case "not support":
            case "error":
                ...
                var result = event.getCurrentTarget().dollar.Recognize(event.getCurrentTarget().recognizer.getPoints(), 1);
                cc.log(result.Name);
                if (result.Name == "circle") {
                    event.getCurrentTarget().runner.incredibleHulk();
                }
                break;
        }
    },
...
PlayLayer.scene = function () {
    ...
    scene.addChild(statusLayer, 0,  1);
    return scene;
};
